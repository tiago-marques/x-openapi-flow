...
- **Python SDK generation** (`x-openapi-flow generate-sdk --lang python`): generates the same flow-aware, lifecycle-gated SDK shape as the TypeScript generator — a `{Resource}ResourceInstance` base class, one subclass per lifecycle state with transition methods, and a `{Resource}Resource` service class with auto-prerequisite resolution — from the same language-agnostic intermediate model. Output is plain, synchronous, stdlib-only Python (a `urllib`-backed `HttpClient`, no third-party dependencies, no `asyncio`). `--lang typescript|python` replaces the previous TypeScript-only restriction.
- **`x-openapi-flow export-llm-flows` command** (`adapters/docs/llm-adapter.js`) and matching `export_llm_flows` MCP tool: exports a machine-oriented flow contract intended for a coding agent implementing an integration, not for human reading. Grouped by resource: `initial_states`, `terminal_states`, `entry_points` (operations with no prerequisites), and per operation its `method`/`path`/`path_params` plus every authored transition field verbatim — `decision_rule`, `evidence_refs`, `prerequisite_field_refs`, `propagated_field_refs`, `failure_paths`, `compensation_operation_id`, `async_contract`, `transition_priority` — none of which survive into `export-doc-flows`'s JSON model. Defaults to a compact `yaml` format (also supports `json`); intentionally omits request/response schemas already covered by the base OpenAPI file.
