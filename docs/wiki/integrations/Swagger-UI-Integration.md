...
- `example/swagger-ui/examples/swagger-ui/index.html`
- `x-openapi-flow/adapters/ui/swagger-ui/x-openapi-flow-plugin.js`
...
`http://localhost:8080/example/swagger-ui/examples/swagger-ui/index.html`

## How it works

- `showExtensions: true` keeps vendor extensions visible.
- Custom plugin adds lifecycle details in operation summary with:
  - `version`
  - `current_state`
- Plugin also renders a global Flow Overview section (Mermaid) when lifecycle transitions are available.
