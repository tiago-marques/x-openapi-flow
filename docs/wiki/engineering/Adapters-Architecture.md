...
Browser-side plugin scripts (`swagger-ui/`, `redoc/`) are standalone files loaded directly in HTML pages. They do not have Node.js dependencies and must be self-contained. Test them with `tests/plugins/plugin-ui.test.js` and `tests/plugins/redoc-plugin.test.js` using a mock browser environment.

## Related

- [CLI Reference](../reference/CLI-Reference.md) — CLI flags and examples for each adapter command.
- [Integration Testing](Integration-Testing.md) — how to write cross-adapter integration tests.
- [SDK Example (TypeScript)](../../../example/sdk/typescript/README.md) — generated SDK usage and workflow example.
