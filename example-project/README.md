...
- `swagger-openapi-flow.json`: full output generated by `apply`
...
To load the full spec generated by the library, set:

```bash
SWAGGER_SPEC_URL=/swagger-openapi-flow.json npm start
```
...
- `apply`: merges `examples/order-openapi-flow.yaml` into `swagger.json` and writes `swagger-openapi-flow.json`.
- `validate` and `graph`: run on the generated full output.
